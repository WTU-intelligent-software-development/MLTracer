...
def cv(name, feature_path, dbm=None):
    ...
    df = pd.read_excel(os.path.join(feature_path, "Boruta/Boruta_selected_features.xlsx"), index_col=0)
    ...
    logdir = os.path.join(root, "outputs/logs/", name.split("-")[0], "Boruta_select",
                          name.split("-")[1] + ".log")  # ./outputs/logs/UC_CC/None.log
    log = logger(logdir, name="Boruta")
...
def write_to_excel(output_fname=os.path.join(root, "outputs/performance/")):
    if output_fname is not None:
        ...
        result.to_excel(os.path.join(output_fname + "scores_Boruta.xlsx"))
